...
Lexer.prototype.token = function (src, top, bq) {
  ...
  while (src) {
    ...
    if (cap = this.rules.list.exec(src)) {
      var _last = this.tokens[this.tokens.length - 1];
      if (_last && _last.type === "paragraph" && _last.text === "") {
        this.tokens.splice(-1, 1);
      }
    ...
    if (top && (cap = this.rules.paragraph.exec(src))) {
      ...
      var _last2 = this.tokens[this.tokens.length - 1];
      if (_last2 && _last2.type === "paragraph" && _last2.text === "") {
        this.tokens.splice(-1, 1);
      }
    ...
    case "blockquote_start":
      {
        ...
        while (this.next().type !== "blockquote_end") {
          // For multi-line blockquotes, nodes are paragraphs so we need to make sure
          // to add a newline in between the generated text nodes
          var text = "" + (first ? '' : '\n') + this.token.text;
          _body.push(this.inline.parse(text));
          first = false;
        }
